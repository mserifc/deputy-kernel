...
void protect_init() {
    protect_GDTPointer.limit = (sizeof(struct protect_GDTEntry_t) * 3) - 1; // Set the size of the GDT (total size of all entries minus one)
    protect_GDTPointer.base = (uint32_t)&protect_GDTEntry;                  // Set the base address of the GDT (the address of gdt_entry array)
