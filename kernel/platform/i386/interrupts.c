...
// The exception handler that gets called for each exception interrupt
...
void interrupts_PICSendEOI(uint8_t irq) {
	if(irq >= 8) {                      // If the IRQ is above 7, also send EOI to Slave PIC
		port_outb(INTERRUPTS_PIC_SLAVE_CMD, INTERRUPTS_PIC_EOI);   // Send EOI to Slave PIC
    }
	port_outb(INTERRUPTS_PIC_MASTER_CMD, INTERRUPTS_PIC_EOI);       // Send EOI to Master PIC
}

// Read the Interrupt Request Register (IRR) from the PIC
uint16_t interrupts_PICGetIRR(void) {
    port_outb(INTERRUPTS_PIC_MASTER_CMD, INTERRUPTS_PIC_IRR);  // Request IRR from Master PIC
    port_outb(INTERRUPTS_PIC_SLAVE_CMD, INTERRUPTS_PIC_IRR);  // Request IRR from Slave PIC
    return (port_inb(INTERRUPTS_PIC_SLAVE_CMD) << 8) | port_inb(INTERRUPTS_PIC_MASTER_CMD);  // Return the combined result
}

// Read the In-Service Register (ISR) from the PIC
uint16_t interrupts_PICGetISR(void) {
    port_outb(INTERRUPTS_PIC_MASTER_CMD, INTERRUPTS_PIC_ISR);  // Request ISR from Master PIC
    port_outb(INTERRUPTS_PIC_SLAVE_CMD, INTERRUPTS_PIC_ISR);  // Request ISR from Slave PIC
    return (port_inb(INTERRUPTS_PIC_SLAVE_CMD) << 8) | port_inb(INTERRUPTS_PIC_MASTER_CMD);  // Return the combined result
}
...
void interrupts_PICIRQEnable(uint8_t irq) {
    ...
    if (irq < 8) {          // If the IRQ is less than 8, it's on the Master PIC
        port = INTERRUPTS_PIC_MASTER_DATA;
    } else {                // Otherwise, it's on the Slave PIC
        port = INTERRUPTS_PIC_SLAVE_DATA;
...
void interrupts_PICDisable(void) {
    port_outb(INTERRUPTS_PIC_MASTER_DATA, 0xFF);     // Mask all IRQs on Master PIC
    port_outb(INTERRUPTS_PIC_SLAVE_DATA, 0xFF);     // Mask all IRQs on Slave PIC
}
...
void interrupts_PICRemap(uint8_t offset1, uint8_t offset2) {
    ...
    // Save Masks
    p1 = port_inb(INTERRUPTS_PIC_MASTER_DATA);                   // Get Master PIC's Interrupt Masks
    p2 = port_inb(INTERRUPTS_PIC_SLAVE_DATA);                   // Get Slave PIC's Interrupt Masks

    // Start PIC (ICW1)
    port_outb(INTERRUPTS_PIC_MASTER_CMD, INTERRUPTS_PIC_ICW1_INIT | INTERRUPTS_PIC_ICW1_ICW4); // Master PIC
    port_ioWait();
    port_outb(INTERRUPTS_PIC_SLAVE_CMD, INTERRUPTS_PIC_ICW1_INIT | INTERRUPTS_PIC_ICW1_ICW4); // Slave PIC
    port_ioWait();

    // Set Interrupt Vector Offsets (ICW2)
    port_outb(INTERRUPTS_PIC_MASTER_DATA, offset1);              // IRQ0 - IRQ7
    port_ioWait();
    port_outb(INTERRUPTS_PIC_SLAVE_DATA, offset2);              // IRQ8 - IRQ15
    port_ioWait();

    // Connect Slave PIC to Master PIC (ICW3)
    port_outb(INTERRUPTS_PIC_MASTER_DATA, 0x04);                 // Master PIC informs the Slave PIC is connected
    port_ioWait();
    port_outb(INTERRUPTS_PIC_SLAVE_DATA, 0x02);                 // Slave PIC connects to Master PIC
    port_ioWait();

    // Set Operating Mode (ICW4)
    /*
     ! Attention! There is an important personal preference here.
     *
     * By activating auto mode, we ensure PIC automatically sends the EOI signal ('INTERRUPTS_PIC_ICW4_AUTO' option)
     *
     * You may think that this will be a problem because while the interrupt is being processed, another interrupt may come
     * and ruin the process, but I realized that when a PIC interrupt comes, the interrupts are disabled and we need to use 
     * the STI command. That's why we do this to avoid any extra code load.
     * 
     * If you want to use normal mode, remove 'INTERRUPTS_PIC_ICW4_AUTO' option.
     */
    port_outb(INTERRUPTS_PIC_MASTER_DATA, INTERRUPTS_PIC_ICW4_8086 | INTERRUPTS_PIC_ICW4_AUTO);     // Set to 8086 Mode (with auto)
    port_ioWait();
    port_outb(INTERRUPTS_PIC_SLAVE_DATA, INTERRUPTS_PIC_ICW4_8086 | INTERRUPTS_PIC_ICW4_AUTO);      // Set the same Mode
    port_ioWait();

    // Restore Masks
    port_outb(INTERRUPTS_PIC_MASTER_DATA, p1);                   // Restore Saved Master PIC's Interrupt Masks
    port_outb(INTERRUPTS_PIC_SLAVE_DATA, p2);                   // Restore Saved Slave PIC's Interrupt Masks
}
...
void interrupts_init() {
    ...
    interrupts_PICRemap(
        INTERRUPTS_PIC_MASTER_OFFSET,
        INTERRUPTS_PIC_SLAVE_OFFSET
    );
